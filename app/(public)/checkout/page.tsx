          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              ...
              >
                ...
                <motion.button
                  onClick={goToPayment}
                  className="group relative mt-8 h-14 w-full overflow-hidden rounded-full bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-white shadow-xl transition-all hover:shadow-2xl md:mt-12 md:h-16"
                  whileHover={{ scale: 1.02, y: -2 }}
                  whileTap={{ scale: 0.98 }}
                >
                  {/* Animated gradient overlay */}
                  <div className="absolute inset-0 bg-gradient-to-r from-amber-600 via-yellow-500 to-amber-600 opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
                  
                  {/* Button content */}
                  <div className="relative z-10 flex items-center justify-center gap-3 text-xs font-bold uppercase tracking-[0.25em] md:text-sm">
                    Continue to Payment
                    <motion.div 
                      animate={{ x: [0, 5, 0] }} 
