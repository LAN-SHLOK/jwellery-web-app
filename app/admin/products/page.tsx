...
const EMPTY_FORM: ProductFormState = {
  ...
  gold_weight_grams: '',
  gold_purity: '22K',
  available_in_both_purities: false,
  hallmark_number: '',
  imageBase64: '',
  jeweller_margin: '',
...
export default function ProductManagement() {
  ...
  function openEdit(product: ProductRecord) {
    setFormData({
      ...
      gold_purity: product.gold_purity || '22K',
      available_in_both_purities: product.available_in_both_purities || false,
    ...
    // Scroll drawer to top when opened
    setTimeout(() => {
      const drawer = document.querySelector('aside .overflow-y-auto');
      if (drawer) drawer.scrollTop = 0;
    }, 100);
  }
                          ...
                          <div className="mt-4 space-y-4">
                            <div className="space-y-4">
                              <div className="rounded-[22px] border border-brand-text/8 bg-brand-background/80 px-4 py-4">
                                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-brand-text/34">Gold Purity</p>
                                <select
                                  value={formData.gold_purity}
                                  onChange={(event) =>
                                    setFormData((current) => ({ ...current, gold_purity: event.target.value as '18K' | '22K' }))
                                  }
                                  className="mt-2 w-full rounded-lg border border-brand-text/8 bg-white px-3 py-2 font-semibold text-brand-primary outline-none transition focus:border-brand-accent/40"
                                >
                              ...
                              <div className="rounded-[22px] border border-brand-text/8 bg-brand-background/80 px-4 py-4">
                                <label className="flex items-center gap-3 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    checked={formData.available_in_both_purities}
                                    onChange={(event) =>
                                      setFormData((current) => ({ ...current, available_in_both_purities: event.target.checked }))
                                    }
                                    className="h-5 w-5 rounded border-brand-text/20 text-brand-accent focus:ring-brand-accent/30"
                                  />
                                  <div>
                                    <p className="text-sm font-semibold text-brand-primary">Available in both purities</p>
                                    <p className="mt-1 text-xs text-brand-text/60">Allow customers to choose between 18K and 22K</p>
                                  </div>
                                </label>
                              </div>
                            </div>
